...
void MagixOwnershipManager::initialize(MagixExternalDefinitions *def, MagixGameStateManager *gameStateMgr, MagixCritterManager *critterMgr, MagixUnitManager *unitMgr, MagixNetworkManager *networkMgr, MagixWorld *world, MagixChatManager *chatMgr, MagixSkyManager *skyMgr)
{
	mDef = def;
...
void MagixOwnershipManager::updateCritters(const FrameEvent &evt)
{
	...
	for (int i = 0; i<(int)tCritterDecisionList.size(); i++)
	{
		...
		else
		{
			...
			tC->setDecisionTimer(Math::RangeRandom(tInfo.decisionMin, (Real)(tInfo.decisionMin + tInfo.decisionDeviation)));
		}
	}
