...
void MagixMiniMap::initialize(MagixExternalDefinitions *def, MagixUnitManager *unitMgr, MagixCollisionManager *collisionMgr, MagixCamera *camera)
{
	mDef = def;
...
void MagixMiniMap::update(bool cursorVisible, const Real &cursorX, const Real &cursorY, const FrameEvent &evt)
{
...
bool MagixMiniMap::mapPosition(const Vector2 &center, const Vector2 &input, Vector2 &output)
{
	output = Vector2((Real)((center.x - input.x)*0.00006*zoomRatio + 0.06), (Real)((center.y - input.y)*0.00008*zoomRatio + 0.08));
...
void MagixMiniMap::zoomIn()
{
	...
	mMapCone->setPosition((Real)(mMap->getWidth()*0.5 - mMapCone->getWidth()*0.5), (Real)(mMap->getHeight()*0.5 - mMapCone->getHeight()*0.5));
}
